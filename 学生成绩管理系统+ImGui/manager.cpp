...
//4. 向平均分列表中添加0.0f
//5. 向过滤器列表中添加一个空的过滤器
//6. 调用calculateAverageScore()函数，计算平均分 - 无用
...
void Manager::addCourse(string course, vector<float> courseScores, bool undoable)
{
    ...
    averageScores.push_back(averageScore); //添加平均分
    Course_filter filter;
    table.filter.course_filters.push_back(filter); //添加过滤器
    if (undoable)
        history.addAction(AddCourse, course, course);
    //calculateAverageScore(); 避免重复计算
...
//2. 然后删除课程
//3. 调用calculateAverageScore()函数，计算平均分 - 直接减去该课程分数，避免重复计算
...
int Manager::modifyCourse(string course, string newCourse, bool undoable)
{
    ...
    if (course.empty()) {
        return 3; //TODO: 查找一个名字为空的课程，显然不存在
    }
    if (newCourse == course) {
        return 4; //TODO: 无需做无用功
    }
    //检测课程是否已经存在
    //for (int i = 0; i < courses.size(); i++)
    //{
    ...
    //    }
    //}
    if (courses.contains(newCourse)) return 2; // 课程已经存在
    int index = -1;
    for (int i = 0; i < courses.size(); i++)
    {
        if (courses[i] == course)
...
//3. 修改课程名
//4. 调用calculateAverageScore()函数，计算平均分 - 无用，改变名字并不改变成绩
...
//3. 然后清空学生的分数列表
//4. 然后清空平均分列表
//5. 调用calculateAverageScore()函数，计算平均分 - 无用
...
int Manager::addStudent(Student student, bool undoable)
{
    ...
    for (int i = 0; i < courses.size(); i++) 
    {
        ...
        if (score < 0.0f && score != -1.0f) // 可维护程度更高
        {
            return 2; //分数不合法
        }
    }
...
{
    students.clear();
    calculateAverageScore(); // 有空处理，正确的
    history.clear();
}
